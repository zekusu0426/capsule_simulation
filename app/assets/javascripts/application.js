...
$(function() {
    ...
    //     }
    // });
    $(".btn-select_all").on('click', function(){ //全選択・全解除をクリックしたとき
        $(this).closest('.modal-body').find('input').prop('checked', true);
    });

    $(".btn-all_cancellation").on('click', function(){ //全選択・全解除をクリックしたとき
        $(this).closest('.modal-body').find('input').prop('checked', false);
    });

    $(".btn-select_all,.btn-all_cancellation").on('click', function() {
        var cnt = $('#card_list .card_checkbox input:checkbox:checked').length;
        $('div.possession_rate').text('所持率：' + Math.round(cnt*100*100/total)/100 + '％' );
        $('div.selected_card').text('選択：' + cnt + '個');
    }).trigger('change');   
